...
function rolePage(role, config) {
  const { folder, requireRole, nav, pages, dashboardBody } = config;
  const base = `../`;
  ...
  function shell(p, content) {
    const i18nAttr = p.i18n ? ` data-i18n="${p.i18n}"` : "";
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body data-require-role="${requireRole}">
    ...
    <div class="main-wrap">
      <header class="top-header">
        <div class="header-left">
          ...
          <h1 class="page-title"${i18nAttr}>${p.title}</h1>
        </div>
        <div class="header-right">
          ...
          <div class="user-menu">
            ...
            <div class="dropdown"><a href="${folder === "student" ? "profile.html" : "settings.html"}">Profile</a><button type="button" data-logout>Log out</button></div>
          </div>
        </div>
      </header>
  ...
  const dash = {
    file: "dashboard.html",
    title: folder === "student" ? "Student Dashboard" : "Teacher Dashboard",
    i18n: folder === "student" ? "studentDashboard" : "teacherDashboard",
  };
